...
namespace AnCO {
    ...
    node_ptr memgraph::get_node(const _t_node_id& node_id) {
        return this->graph::get_node(node_id, true);
        }
    int memgraph::get_edges(const _t_node_id& node_id, std::pair<std::vector<edge_ptr>, std::vector<edge_ptr>>& edges) {
