
#pragma once

namespace AnCO {

    namespace GLOBALS {

        const static unsigned int n_max_colonies = N_MAX_COLONIES;
        const static unsigned int n_colonies = N_MAX_COLONIES/10;
        const static unsigned int n_max_ants_per_colony = N_MAX_ANTS_PER_COLONY;
        const static unsigned int n_ants_per_colony = N_MAX_ANTS_PER_COLONY/10;

        //const static unsigned int max_steps = 100;
        const static float min_pheromone = 0.1f;
        const static float max_pheromone = 10.f;

        }

    }